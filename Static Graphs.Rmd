...
quartz(file="Nixon.app.rate", type=pdf, width=4.86, height=9.3)
plot(Nixon.app.rate) 
dev.off()
```
```{r, echo=FALSE}
Clinton.app.rate <- ggplot() + 
  ...
  theme(legend.title=element_blank(), legend.position = "bottom")
Clinton.app.rate
dev.copy2pdf(file="Clinton-approval.pdf")
```
...
quartz(file="grid-partyID", type=pdf, height=155/25.4, width=186/25.4)
dev.off()
```
...
quartz(file="TrumpIpp", type = pdf, width=4.86, height=9.3)
plot(Trump.app.comp) 
dev.off()
