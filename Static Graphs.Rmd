...
ggplot()+ geom_rect(data=nixon, aes(ymin= -Inf, ymax = Inf, xmin=as.Date("1973-10-30", "%Y-%m-%d"), xmax=as.Date("1974-07-30", "%Y-%m-%d")), fill="#FF9999", inherit.aes = FALSE, alpha=0.008) + geom_line(data=nixon, aes(x=midDate, y=approve)) + ggtitle("Nixon") + xlab("Year") + ylab("Approval Rating (%)") + scale_x_date(date_labels= "%y", date_breaks = "1 year")  + scale_y_continuous(limits = c(20, 80)) +  annotate("text", x=as.Date("1974-04-01", "%Y-%m-%d"), y=75, label="House Judiciary Committee\n investigation and hearings ", size=2.5) + geom_line()
...
class(Napp.pid$date)
xstart <- as.POSIXct("1973-10-30")

```


```{r}
ggplot()  +  geom_rect(data=Napp.pid, aes(ymin= -Inf, ymax = Inf, xmin=as.POSIXct("1973-10-30"), xmax=as.POSIXct("1974-07-30")), fill="#FF9999", inherit.aes = FALSE, alpha=0.008) + geom_line(data=Napp.pid, aes(x=date, y=dem_app), color=ussc_darkBlue) + geom_line(data=Napp.pid, aes(x=date, y=ind_app), color="gray") + geom_line(data=Napp.pid, aes(x=date, y=rep_app), color=ussc_red) + ggtitle("Nixon")  + xlab("Year") + ylab("Approval rating by\n party identification") + scale_y_continuous(limits = c(0,100)) + annotate(data = nixon, "text", x=as.POSIXct("1974-03-06 UTC"), y=75, label="House Judiciary Committee\n investigation and hearings", size=2.5)
```
...
ggplot() + geom_rect(data=Capp.pid, aes(ymin= -Inf, ymax = Inf, xmin=as.POSIXct("1998-08-09"), xmax=as.POSIXct("1999-01-06")), fill="#FF9999", inherit.aes=FALSE, alpha=0.008) + geom_line(data=Capp.pid, aes(x=date, y=dem_app), color=ussc_darkBlue) + geom_line(data=Capp.pid, aes(x=date, y=ind_app), color="gray") + geom_line(data=Capp.pid, aes(x=date, y=rep_app), color=ussc_red) + ggtitle("Clinton")  + xlab("Year") + ylab("Approval rating by\n party identification") + scale_y_continuous(limits = c(0,100)) +  annotate(data = Capp.pid, "text",x=as.POSIXct("1999-01-06"), y=99, label="Impeachment deliberation\n and Senate trial", size=2.5)
...
ggplot() + geom_line(data = comp.app.pid, aes(x=dop, y=inparty_app, group=presName)) + geom_line(data=Trump_app, (aes(x=dop, y=inparty_app, size=10, color=ussc_red_alpha))) + xlab("Days of Presidency ") + ylab("In-party approval rating")+ theme_bw() + theme(legend.position = "none") + ggtitle("Trump approval among in-party partisans, compared with Eisenhower to Obama")
```
